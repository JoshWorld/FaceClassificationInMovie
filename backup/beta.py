    ...
    with tf.Session(graph=detection_graph, config=config) as sess:
        with CustomObjectScope({'tf': tf}):
            ...
            while True:
                    ...
                    for face_idx, face in enumerate(face_list):
                        ...
                        for group_idx, group in enumerate(face_group):
                            ...
                            for item in group:
                                ...
                                point_dis = point_dis + calc_vector_distance(item['center'], face['center'])
                                #m_dis = calc_vector_distance(item['embedding_vector'], face['embedding_vector'])

                            m_dis = dis/len(group)
                            p_m_dis = point_dis/len(group)
                            ...
                            face_info.append({'face_idx': face_idx, 'group_idx': group_idx, 'distance': m_dis,
                                              ...
                                              'embedding_vector': face['embedding_vector'], 'center_distance': p_m_dis})
                ...
                from other_function import kmeans_function
