                    ...
                    for item in sort_face:
                        if (face_check[item['face_idx']] == False) and (group_check[item['group_idx']] == False):
                            ...
                            if item['e_distance'] < NEW_FACE_RATE:
                                ...
                                cv2.putText(image, "result: " + str(item['result']) + " / group_idx : " + str(item['group_idx']) + " / distance : " + str(item['e_distance']), tuple(item['max']), font, 0.5, color_list[item['group_idx']], 1, cv2.LINE_AA)
